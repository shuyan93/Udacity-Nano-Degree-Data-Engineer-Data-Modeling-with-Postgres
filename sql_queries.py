...
artist_table_create = ("""CREATE TABLE IF NOT EXISTS artists ( artist_id varchar PRIMARY KEY, artist_name varchar, artist_location varchar, artist_latitude  Decimal(9,6) , artist_longitude Decimal(9,6))""")
...
song_select = ("""SELECT song_id, artists.artist_id from songs join artists on songs.artist_id = artists.artist_id WHERE title = %s and artists.artist_name = %s and songs.duration=%s""")
